...
class RealmExperienceTest {
    ...
    @Test fun selectionsAndTrainingAmountSurviveNavigation() {
        ...
        captureTestScreenshot(compose,"beta5-training-edit")
        compose.onNodeWithContentDescription("زيادة عدد الجنود").performScrollTo().assertIsDisplayed().performClick()
    ...
    @Test fun profileInventoryCouncilAndSceneToolsStayReachable() {
        ...
        compose.onNodeWithText("المستودع").performClick()
        compose.onNodeWithTag("warehouse-search").assertExists()
        compose.onNodeWithText("لا توجد عناصر هنا").performScrollTo().assertIsDisplayed()
        compose.onNodeWithTag("tab-more").performClick()
